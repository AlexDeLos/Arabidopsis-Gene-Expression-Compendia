...
with nav_col1:
    if st.button("⬅️ Previous Sample", use_container_width=True, disabled=(st.session_state.current_idx == 0)):
        st.session_state.current_idx -= 1
        st.rerun()
with nav_col3:
    if st.button("Skip / Next Sample ➡️", use_container_width=True, disabled=(st.session_state.current_idx == len(data) - 1)):
        st.session_state.current_idx += 1
        st.rerun()
