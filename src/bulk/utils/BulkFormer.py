...
class BulkFormer(nn.Module):
    def __init__(self, dim, graph, gene_emb, gene_length, bin_head=4, full_head=4, bins=10, gb_repeat=3, p_repeat=1, use_graph=True):
        super().__init__()
        self.dim = dim
        ...
        # === 样本整体表达 embedding ===
        self.global_expr_proj = nn.Sequential(nn.Linear(gene_length, 4 * dim), nn.ReLU(), nn.Linear(4 * dim, dim))
