...
def analyze_divergence_patterns(dir1, dir2):
    ...
    common_files = {f.name for f in path1.glob("*.json")} & {f.name for f in path2.glob("*.json")}
...
def compare_labels(dir1, dir2):
    ...
    files1 = {f.name for f in path1.glob("*.json")}
    files2 = {f.name for f in path2.glob("*.json")}
