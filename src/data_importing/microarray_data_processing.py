...
from src.data_importing.helpers.download_helper import check_metadata_for_cel, clean_files, count_cel_files  # noqa: E402
...
class Microarray_tracker:
    def __init__(self) -> None:
        ...
        self.totals: dict = {"total_studies_seen": 0, "total_sample_seen": 0, "total_samples_used": 0, "total_studies_used": 0}
    ...
    def update_platform(self, platform: str, samples: int, used: bool):
        ...
        if platform not in self.platform_counts:
            self.platform_counts[platform] = {"studies_seen": 0, "studies_used": 0, "samples_seen": 0, "samples_used": 0}
    ...
    def sync_with_filesystem(self, download_dir: str, processed_dir: str):
        ...
        """
        print("Syncing tracker with files...")
        print(f"  - Scanning Processed: {processed_dir}")
        print(f"  - Scanning Downloads: {download_dir}")
        ...
        for gse in actual_downloaded_ids:
            if gse not in actual_processed_ids and not self.is_downloaded(gse):
        ...
        if not are_equal:
            ...
            for k in keys:
                if self.states[k] != other_tracker.states[k]:
                    ...
                    if diff_1:
                        print(f"  Difference in '{k}': Self has extra {len(diff_1)} items (e.g., {list(diff_1)[:3]}...)")
                    if diff_2:
                        print(f"  Difference in '{k}': Other has extra {len(diff_2)} items (e.g., {list(diff_2)[:3]}...)")
    ...
    def save_to_json(self, filename="tracker_results.json"):
        ...
        data = {"totals": self.totals, "platform_counts": self.platform_counts, "study_tracker": self.study_tracker, "states": serializable_states}
    ...
    def load_from_json(cls, filename="tracker_results.json"):
        ...
        tracker.states = {"ignore": set(loaded_states.get("ignore", [])), "downloaded": set(loaded_states.get("downloaded", [])), "processed": set(loaded_states.get("processed", []))}
...
class Microarray_data_processing:
    ...
    def _map_symbols_to_locus_ids(self, identifiers):
        ...
        if to_query:
            ...
            try:
                results = mg.querymany(to_query, scopes="symbol,alias,name,reporter", fields="locus_tag,ensembl.gene", species="3702", returnall=False, as_dataframe=False, verbose=False)
    ...
    def process_microarray_metadata_and_rma(self, experiment_id, raw_data_folder, output_folder, gse) -> int:
        ...
        if not cel_files:
            msg = f"  - ABORTING: No CEL files found in {raw_data_folder}."
            ...
            try:
                ...
                if platform_id and platform_id in gse.gpls:
                    ...
                    if target_col:
                        ...
                        probe_to_raw_map = dict(zip(valid_annot["ID"], valid_annot["Cleaned_Val"], strict=False))
                        ...
                        msg_0 = f"    > WARNING: Could not find suitable gene column in GPL {platform_id}."
                        raise ValueError(msg_0)
                else:
                    msg_0 = "    > Platform data not found in GSE object."
                    raise ValueError(msg_0)
...
def download_experiments_microarray(
    data_processor: Microarray_data_processing, gse_list, download_dir, tracker: Microarray_tracker, download_raw=True, scan=True, output_folder="./microarray_processed_data/"
):
    """
        ...
        try:
            ...
            if not tracker.is_downloaded(gse_id):
                ...
                if download_raw:
                    ...
                    gse.download_supplementary_files(directory=exp_folder, download_sra=False)  # type: ignore  # noqa: PGH003
